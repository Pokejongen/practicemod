...
{	
	thread common_functions::WriteToScreen("Opening All Doors");
        ...
        foreach(door in doors)
        {
			switch(type)
			{
				case "zombie_airlock_buy":
					...
					break;
				default:
					if(IsDefined(door) && !common_functions::IsTrue(door.has_been_opened)) door notify("trigger", self, 1);
					wait 0.1;
				break;
			}
        }
...
function OpenBeastDoors()
{
	if(level.script != "zm_zod") return;
	if(IsTrue(level.smashables_open))
	{
		thread common_functions::WriteToScreen("Beast Breakables Already Broken");
		return;
	}
	else
	{
		level.smashables_open = 1;
		thread common_functions::WriteToScreen("Opening All Beast Breakables");
	}
...
function ActivateAllPower(excludes = Array())
{
	thread common_functions::WriteToScreen("Turning On All Powers");
...
function SetZBarrierState(state = 0)
{
	if(!state)
	{
		thread common_functions::WriteToScreen("Opening All Barriers");
		zm_blockers::open_all_zbarriers();
	}
	else
	{
		thread common_functions::WriteToScreen("Closing All Barriers");
...
function ActivateSpecificPower(num)
{
	switch(level.script)
	{
		case "zm_zod":
		{
			...
			{
				thread common_functions::WriteToScreen("Power Already Activated");
				return;	
			}
			thread common_functions::WriteToScreen("Activating Selected Power");
		...
		case "zm_genesis":
		{
			...
			{
				thread common_functions::WriteToScreen("Generator Already Activated");
				return;
			}
			thread common_functions::WriteToScreen("Activating Selected Generator");
		...
		case "zm_tomb":
		{
			...
			{
				thread common_functions::WriteToScreen("Generator Already Captured");
				return;
			}
			thread common_functions::WriteToScreen("Capturing Selected Generator");
			//generator [[ @zm_tomb_capture_zones<scripts\zm\zm_tomb_capture_zones.gsc>::set_player_controlled_area ]]();
			generator.n_current_progress = 100;
			break;
		}
...
function ActivateMapSpecificObject(num)
{
	switch(level.script)
	{
		case "zm_zod":
		{
			...
			{
				thread common_functions::WriteToScreen("Smashable Already Smashed");
				return;
			}
			thread common_functions::WriteToScreen("Smashing Selected Smashable");
		...
		case "zm_factory":
		{
			...
			{
				thread common_functions::WriteToScreen("Power Must Be Turned On");
				return;
			}
			if(level.teleport[num] != "waiting")
			{
				thread common_functions::WriteToScreen("Teleporter Already Active");
				return;
			}
			thread common_functions::WriteToScreen("Activating Selected TP");
		...
		case "zm_castle":
		{
			...
			{
				thread common_functions::WriteToScreen("Landing Pad Already Activated");
				return;
			}
			foreach(stub in level._unitriggers.trigger_stubs)
			{
				if(IsDefined(stub.flag_name) && stub.flag_name == pad.script_noteworthy)
				{
					thread common_functions::WriteToScreen("Activating Selected Landing Pad");
					self BuildAndActivateTrigger(stub);
					break;
				}
			}
		...
		case "zm_genesis":
		{
			...
			{
				thread common_functions::WriteToScreen("Apothicon Allreadt Trapped");
				return;	
			}
			thread common_functions::WriteToScreen("Trapping Apothicon");
			self thread TrapApothicon();
			break;
		}
		case "zm_theater":
		{
			if(!level flag::get("power_on"))
			{
				thread common_functions::WriteToScreen("Power Must Be On");
				return;
			}
			if(level flag::get("teleporter_linked"))
			{
				thread common_functions::WriteToScreen("Teleporter Already Linked");
				return;
			}
			thread common_functions::WriteToScreen("Linking TP");
		...
		case "zm_cosmodrome":
		{
			...
			{
				thread common_functions::WriteToScreen("PAP Already Opened");
				return;
			}
			thread common_functions::WriteToScreen("Opening PAP");
		...
		case "zm_temple":
		{
			...
			{
				thread common_functions::WriteToScreen("PAP Already Activated");
				return;
			}
			thread common_functions::WriteToScreen("Activating PAP");
		...
		case "zm_moon":
		{
			if(level flag::get("enter_nml"))
			{
				...
				{
					thread common_functions::WriteToScreen("Gate Already Opened");
					return;
				}
				thread common_functions::WriteToScreen("Opening TP Gate");
...
function OpenDoorByFlagOrOrigin(flag_org)
{
	if(IsString(flag_org))
	{
		...
		foreach(type in types)
		{
			...
			foreach(door in doors)
			{
				...
				{
					thread common_functions::WriteToScreen("Door Already Opened");
					return;
				}
				SetDvar("zombie_unlock_all", 1);
				thread common_functions::WriteToScreen("Opening Selected Door");
	...
	thread common_functions::WriteToScreen("Door Not Found");
}