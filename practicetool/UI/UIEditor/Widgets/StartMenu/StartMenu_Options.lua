...
CoD.StartMenu_Options.new = function ( menu, controller )
    ...
    -- Add text above the round number
	local roundText = LUI.UIText.new()
	roundText:setLeftRight(true, false, 840, 1040) -- Same horizontal position as ZmRnd
	roundText:setTopBottom(true, false, 350, 380) -- Positioned above ZmRnd
	roundText:setText(Engine.Localize("Round"))
	roundText:setTTF("fonts/default.ttf")
	roundText:setAlignment(Enum.LUIAlignment.LUI_ALIGNMENT_CENTER)
	roundText:setRGB(1, 1, 1)
	roundText:setAlpha(0.9)
	self:addElement(roundText)
	self.roundText = roundText

	-- Update text based on pause state
	self:subscribeToModel(Engine.GetModel(Engine.GetModelForController(controller), "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_UI_ACTIVE), function(model)
	    local isPaused = Engine.GetModelValue(model)
	    if isPaused then
	        roundText:setText(Engine.Localize("Paused at Round")) -- Update text when paused
	    else
	        roundText:setText(Engine.Localize("Round")) -- Default text when not paused
	    end
	end)
	
	-- Add the ZmRnd widget for round number display
	local zmRnd = CoD.ZmRnd.new(menu, controller)
	zmRnd:setLeftRight(true, false, 820, 1020)
	zmRnd:setTopBottom(true, false, 310, 350)
